Update the `Models/ForgeService.cs` file with the following code:
...
:::caution
Note that the _Data Management service_ requires bucket names to be **globally unique**,
...
in the preconfigured bucket, and a helper method for uploading additional files to this bucket:
